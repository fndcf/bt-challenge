...
export function LogMethod(operation: string) {
  return function (
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      const controllerName = (this as any).controllerName || "Controller";

      try {
        logger.debug(`${controllerName}: Iniciando ${operation}`);
        const result = await originalMethod.apply(this, args);
        const duration = Date.now() - startTime;
        logger.debug(`${controllerName}: ${operation} concluído`, {
          durationMs: duration,
        });
        return result;
      } catch (error) {
        const duration = Date.now() - startTime;
        logger.error(`${controllerName}: Falha em ${operation}`, {
          durationMs: duration,
        });
        throw error;
      }
